namespace Millwright.ModSystem
{
    ...
    public class BEBehaviorWindmillRotorDouble : BEBehaviorMPRotor
    {
        ...
        private readonly float bladeModifier = (float)ModConfig.Loaded.SailCenteredModifier * 2;
        ...
        public BEBehaviorWindmillRotorDouble(BlockEntity blockentity) : base(blockentity)
        {
        }
        ...
        private void CheckWindSpeed(float dt)
        {
            ...
            if (this.Api.Side == EnumAppSide.Server && this.SailLength > 0 && this.Api.World.Rand.NextDouble() < 0.2)
            {
                if (this.Obstructed(this.SailLength + 1))
                {
                    ...
                    {
                        var stacks = new ItemStack(this.Api.World.GetItem(new AssetLocation("millwright:sailcentered")), 8);
        ...
        internal bool OnInteract(IPlayer byPlayer)
        {
            ...
            if (slot.Empty || slot.StackSize < 8)
            {
                return false;
            }
        ...
        protected override void updateShape(IWorldAccessor worldForResolve)
        {
            ...
            if (this.SailLength == 0)
            {
                ...
                {
                    Base = new AssetLocation("millwright:block/wood/mechanics/double/windmillrotor"),
            ...
            else
            {
                this.Shape = new CompositeShape()
                {
                    Base = new AssetLocation("millwright:block/wood/mechanics/double/centered/windmill-" + this.SailLength + "blade"),
                    rotateY = this.Block.Shape.rotateY
                };
            }
        }
